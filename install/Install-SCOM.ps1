...
{
    $addedfeature = Install-WindowsFeature -Name 'NET-Framework-45-Core';
};
...
if($Components -match 'OMServer')
{
    ...
    if($FirstManagementServer)
    {
        ...
        if($DWSqlServerInstance -and ($SqlServerInstance -ne $DWSqlServerInstance))
        {
            ...
            $connectionstringBase = 'Data Source='+$DWSqlServerInstance+','+$DWSqlInstancePort+';Persist Security Info=True;Initial Catalog=OperationsManagerDW;UID='
            # Test connecting to SQL Server Instance with DAS Account and Password
            $connectionstring = $connectionstringBase+$($DASAccountUser.UserName)+';PWD='+$dasAccountPassword;
            $sqlconnection = Test-DataSource -ConnectionString $connectionstring;
            if(-not($sqlconnection.Connected))
            {
                throw "Unable to connect to OperationsManagerDW database on DW Sql Server Instance $DWSqlServerInstance,$DWSqlInstancePort with DASAccountUser $($dasAccountUser.UserName)!";
            };
            # Test connecting to SQL Server Instance with DataReader Account and Password
            $connectionstring = $connectionstringBase+$($DataReaderUser.UserName)+';PWD='+$dataReaderPassword;
            $sqlconnection = Test-DataSource -ConnectionString $connectionstring;
            if(-not($sqlconnection.Connected))
            {
                throw "Unable to connect to OperationsManagerDW database on DW Sql Server Instance $DWSqlServerInstance,$DWSqlInstancePort with DataReaderUser $($DataReaderUser.UserName)!";
            };
        };
        $arglist += @(
...
if($Components -match 'OMWebConsole')
{
    ...
    if($installfeatures)
    {
        $addedfeature = Install-WindowsFeature -Name $installfeatures -Restart;
        return;
    };
...
if($Components -match 'OMReporting' -and $SRSInstance)
{
    ...
    $connectionstringBase = 'Data Source='+$SRSInstance+';Persist Security Info=True;UID='
    # Test connecting to SQL Server Instance with DataReader Account and Password
    $connectionstring = $connectionstringBase+$($DataReaderUser.UserName)+';PWD='+$dataReaderPassword;
    $sqlconnection = Test-DataSource -ConnectionString $connectionstring;
    if(-not($sqlconnection.Connected))
    {
        throw "Unable to connect to Sql Server Reporting Services Instance $SRSInstance with DataReaderUser $($DataReaderUser.UserName)!";
    };
    $arglist += @(
